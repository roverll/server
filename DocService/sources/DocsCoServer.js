...
function sendReleaseLock(conn, userLocks) {
    sendData(conn, {
        type: "releaseLock", locks: _.map(userLocks, function (e) {
            return {
                block: e.block,
                user: e.user,
                time: Date.now(),
...
function* onReplySendStatusDocument(docId, replyData) {
    ...
    if (!(oData && commonDefines.c_oAscServerCommandErrors.NoError == oData.error)) {
        ...
        yield* publish({
            type: commonDefines.c_oPublishType.warning,
            docId: docId,
            description: 'Error on save server subscription!'
        });
    }
}
...
exports.install = function (server, callbackFunction) {
                ...
                try {
                    ...
                    switch (data.type) {
                        ...
                        case 'unSaveLock'      :
                            ...
                            break;	//原 索引被发送-1，因为 这是一次没有保存的紧急撤离
                        case 'getMessages'      :
    ...
    function* closeDocument(conn, isCloseConnection) {
        ...
        if (!reconnected) {
            ...
            yield* publish({
                ...
                docId: docId,
                userId: tmpUser.id,
                participantsTimestamp: participantsTimestamp,
    ...
    function* getDocumentChanges(docId, optStartIndex, optEndIndex) {
        ...
        for (j = 0; j < arrayElements.length; ++j) {
            ...
            objChangesDocument.push({
                docid: docId, change: element['change_data'],
    ...
    function* onMessage(conn, data) {
        ...
        yield* publish({
            type: commonDefines.c_oPublishType.message,
            docId: docId,
            userId: userId,
            messages: messages
        }, docId, userId);
    ...
    function* onCursor(conn, data) {
        ...
        var messages = [msg];
        yield* publish({
            type: commonDefines.c_oPublishType.cursor,
            docId: docId,
            userId: userId,
            messages: messages
        }, docId, userId);
    ...
    function* getLockWord(conn, data, bIsRestore) {
        ...
        yield* publish({
            type: commonDefines.c_oPublishType.getLock,
            docId: docId,
            userId: userId,
    ...
    function* saveChanges(conn, data) {
        ...
        if (0 < newChanges.length) {
            ...
            for (let i = 0; i < newChanges.length; ++i) {
                ...
                arrNewDocumentChanges.push({
                    docid: docId, change: JSON.stringify(oElement), time: newChangesLastTime,
        ...
        if (data.endSaveChanges) {
            ...
            if (!checkEndAuthLockRes) {
                ...
                yield* publish({
                    type: commonDefines.c_oPublishType.changes, docId: docId, userId: userId,
                    changes: changesToSend, startIndex: startIndex, changesIndex: puckerIndex,
                    locks: arrLocks, excelAdditionalInfo: data.excelAdditionalInfo
                }, docId, userId);
            }
            //原 自动删除锁定并发送索引进行保存
            ...
            let isPublished = yield* publish({
                type: commonDefines.c_oPublishType.changes, docId: docId, userId: userId,
                changes: changesToSend, startIndex: startIndex, changesIndex: puckerIndex,
                locks: [], excelAdditionalInfo: undefined
            }, docId, userId);
    ...
    function* unSaveLock(conn, index, time) {
        ...
        //原 ToDo check null === saveLock是一个存根，用于将第二个用户连接到文档（此时没有进行savingLock，但它正在保存，您需要将其删除）
    ...
    sockjs_echo.installHandlers(server, {
        prefix: '/doc/[' + constants.DOC_ID_PATTERN + ']*/c', log: function (severity, message) {
            ...
            if (commonDefines.c_oAscServerCommandErrors.NoError === result && null == docId && 'version' != params.c) {
                ...
                switch (params.c) {
                    ...
                    case 'drop':
                        if (params.userid) {
                            yield* publish({
                                type: commonDefines.c_oPublishType.drop,
                                docId: docId,
                                users: [params.userid],
